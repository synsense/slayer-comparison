...
class SlayerNetwork(pl.LightningModule):
    def __init__(
        ...
        if architecture == "small":
            self.fc1 = self.slayer.dense((34, 34, 2), 500, weightScale=1)
            self.fc2 = self.slayer.dense((1, 1, 500), 100, weightScale=1)
            self.fc3 = self.slayer.dense((1, 1, 100), 10, weightScale=1)
            ...
            self.fc1 = torch.nn.utils.weight_norm(
                self.slayer.dense((6, 6, 64), 10, weightScale=1), name="weight"
            )
        ...
        if architecture in ['paper', 'larger']:
    ...
    def forward(self, x):
        ...
        if self.architecture == "small":
            out = self.slayer.spike(self.slayer.psp(self.fc1(x)))
            out = self.slayer.spike(self.slayer.psp(self.fc2(out)))
            out = self.fc3(out)  
        elif self.architecture == "paper":
