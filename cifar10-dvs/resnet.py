...
class Repeat(nn.Module):
    ...
    def forward(self, x):
        x = x.unflatten(0, (self.batch_size, -1))
        x = x.moveaxis(1, 0)
        output = []
        for step in x:
            output.append(self.module(step))
        output = torch.stack(output).moveaxis(0, 1).flatten(0, 1)
        return output
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.bn1 = Repeat(norm_layer(planes), batch_size=batch_size)
        self.sn1 = spiking_neuron(**kwargs, batch_size=batch_size)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = Repeat(norm_layer(planes), batch_size=batch_size)
        self.sn2 = spiking_neuron(**kwargs, batch_size=batch_size)
        self.downsample = downsample
        if downsample is not None:
            self.downsample_sn = spiking_neuron(**kwargs, batch_size=batch_size)
        self.stride = stride
        self.cnf = cnf
...
class SEWResNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.bn1 = Repeat(norm_layer(self.inplanes), batch_size=batch_size)
        self.sn1 = spiking_neuron(**kwargs, batch_size=batch_size)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(
            block, 64, layers[0], cnf=cnf, spiking_neuron=spiking_neuron, batch_size=batch_size, **kwargs
        )
        ...
        **kwargs,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                Repeat(norm_layer(planes * block.expansion), batch_size=batch_size),
            )
...
def _sew_resnet(arch, block, layers, cnf, batch_size, spiking_neuron, **kwargs):
    model = SEWResNet(
        block,
        layers,
        cnf=cnf,
...
def sew_resnet18(cnf: str, batch_size: int, spiking_neuron: callable = None, **kwargs):
    """
    ...
    return _sew_resnet(
        "resnet18", BasicBlock, [2, 2, 2, 2], cnf, batch_size, spiking_neuron, **kwargs
    )
